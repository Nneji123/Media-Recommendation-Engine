...
try:
    ...
    # app design
    app_meta('🖼️')
    set_bg_hack('./images/comics.png')

    # hide warning for st.pyplot() deprecation
    st.set_option('deprecation.showPyplotGlobalUse', False)

    # Main panel setup
    display_app_header(main_txt='Comic Recommendation App',
                       sub_txt='Get suggestions on what to read to next using this recommendations web app!')

    st.markdown("""---""")

    st.write("""
        ## About
        This App using machine learning algorithms to suggest what comic you should read next!
        """)
...
input_query = st.text_input("Input the comic you last watched: ")
values = {"comic": input_query}
...
st.write("These are the comic you should read next:")
resp = st.dataframe(resp)


#st.write(f"These are the comic you should read next: {resp}.")
